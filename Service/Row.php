...
class Row extends \Phpfox_Service
{

    protected $_sTable = ':cm_row';
    ...
    protected function getText($aVals, $bUpdate = false)
    {
        ...
        $phrase_var_name = 'row_title_' . md5('Slider Title' . $name . PHPFOX_TIME);
        //Add phrases
        $aText = [];
        ...
        $name = $aVals['subtitle_' . $aLanguages[0]['language_id']];
        $phrase_var_name = 'row_subtitle_' . md5('row Title' . $name . PHPFOX_TIME);
        //Add phrases
        $aText = [];
        foreach ($aLanguages as $aLanguage) {
            if (isset($aVals['title_' . $aLanguage['language_id']]) && !empty($aVals['title_' . $aLanguage['language_id']])) {
                $aText[$aLanguage['language_id']] = $aVals['subtitle_' . $aLanguage['language_id']];
            } else {
                return Phpfox_Error::set((_p('Provide a "{{ language_name }}" name.',
                    ['language_name' => $aLanguage['title']])));
            }
        ...
        $subTitlePhrase = Phpfox::getService('language.phrase.process')->add($aValsPhrase);
    ...
    public function create($aVals)
    {
        ...
        $aInsert['background'] = $aVals['background'] ?: '';
        $aInsert['image_position'] = $aVals['image_position'] ?: 'right';
